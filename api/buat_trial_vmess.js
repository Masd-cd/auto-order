...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // Cek IP di database Redis
        const sudahTrial = await client.get(`limit_trial_${ipPembeli}`);
        if (sudahTrial) {
            await client.quit();
            return res.status(400).json({ status: 'gagal', alasan: 'Limit Trial Habis! IP Anda sudah membuat trial hari ini.' });
        }

        let vpsUrl = '';
        let fetchOptions = {};

        // Logika API untuk VMess Trial
        if (serverId === 'SGDO') {
            vpsUrl = 'http://167.172.73.230/vps/trialvmessall'; 
        ...
        try {
            ...
            if (responseVPS.ok) {
                // Kunci IP pembeli selama 24 Jam (86400 detik)
